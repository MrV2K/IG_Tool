...
; Version 0.8a
;
; Removed all FTP procedures.
; Added HTTP download support.
; Added Player, Chipset and Year Data.
; Moved file data to a status bar at the bottom of the window.
; Fixed file list update after edit.
; Moved database and csv code to be compatible with IGame 2.4.x.
; Improved Show Dupes accuracy by adding path matching.
;
; ====================================================================
;
; Version 0.9a
...
Macro Update_Status_Bar()
  StatusBarText(#MAIN_STATUS,0,UM_Database()\UM_Genre)
  StatusBarText(#MAIN_STATUS,1,UM_Database()\UM_Type)
  StatusBarText(#MAIN_STATUS,2,UM_Database()\UM_Year)
  StatusBarText(#MAIN_STATUS,3,"Players: "+UM_Database()\UM_Players)
EndMacro
  ...
  If response<>#PB_MessageRequester_Cancel And path<>""
    If CreateFile(igfile, path,#PB_Ascii)     
      ForEach UM_Database()
        output$="0"+Chr(59)
        If Short_Names
          output$+#DOUBLEQUOTE$+UM_Database()\UM_Short+#DOUBLEQUOTE$+Chr(59)
        Else
          output$+#DOUBLEQUOTE$+UM_Database()\UM_Name+#DOUBLEQUOTE$+Chr(59)
        EndIf
        output$+#DOUBLEQUOTE$+UM_Database()\UM_Genre+#DOUBLEQUOTE$+Chr(59)
        If Output_Case=1
          output$=LCase(output$)
        EndIf
        If Output_Case=2
          output$=UCase(output$)
        EndIf
        output$+#DOUBLEQUOTE$+UM_Database()\UM_Path+UM_Database()\UM_Slave+#DOUBLEQUOTE$+Chr(59)
        If Not Keep_Data
        ...
        Else
          ...
          output$+UM_Database()\UM_Data_4+Chr(59)
          output$+UM_Database()\UM_Data_5+Chr(59)
          output$+#DOUBLEQUOTE$+#DOUBLEQUOTE$+Chr(59)
        EndIf
        output$+UM_Database()\UM_Year+Chr(59)
        output$+UM_Database()\UM_Players+Chr(59)
        output$+#DOUBLEQUOTE$+UM_Database()\UM_Chipset+#DOUBLEQUOTE$+Chr(59)
  ...
  If CSV_Path<>""
    ...
    If ReadFile(CSV_File,CSV_Path,#PB_UTF8)
      ...
      Repeat
        ...
        CSV_List()=RemoveString(Text_String,#DOUBLEQUOTE$)
      ...
      Else
        ...
        ForEach CSV_List()
          ...
          UM_Database()\UM_Data_5=StringField(Text_Line,9,Chr(59))
          UM_Database()\UM_Data_6=StringField(Text_Line,10,Chr(59))
          UM_Database()\UM_Year=StringField(Text_Line,11,Chr(59))
          UM_Database()\UM_Players=StringField(Text_Line,12,Chr(59))
          UM_Database()\UM_Chipset=StringField(Text_Line,13,Chr(59))
          UM_Database()\UM_Filtered=#False
          UM_Database()\UM_Type="Unknown"
        Next
...
Procedure Filter_List()
  ...
  Protected Previous.s, Prev_Path.s
  ...
  ForEach UM_Database()  
    ...
    If Filter
      If UM_Database()\UM_Name=Previous
        If UM_Database()\UM_Path=Prev_Path
...
Procedure Get_Database()
  ...
  Data_File="IG_Data.dat"
  
  DeleteFile(Home_Path+Data_File)
  
  ReceiveHTTPFile("https://ftp2.grandis.nu/turran/FTP/Non%20Amiga/Tools%20for%20Amiga/IG_Tool/IG_Data.dat", Home_Path+Data_File)
  ReceiveHTTPFile("https://ftp2.grandis.nu/turran/FTP/Non%20Amiga/Tools%20for%20Amiga/IG_Tool/genres", Home_Path+"genres")
  
  If FileSize(Home_Path+Data_File)<=0   
    MessageRequester("Error","Download Error. Missing Data file.",#PB_MessageRequester_Error|#PB_MessageRequester_Ok)
  EndIf
  
  If FileSize(Home_Path+"genres")<=0   
    MessageRequester("Error","Download Error. Missing Genres file.",#PB_MessageRequester_Error|#PB_MessageRequester_Ok)
  EndIf
...
Procedure Load_DB()
  ...
  Protected NewMap Compare_Map.s()  
  
  path=Home_Path+Data_File
  ...
  If path<>""
    ...
    If ReadFile(CSV_File,Path,#PB_Ascii)
      ...
      ForEach DB_List()
        ...
        Comp_Database()\C_Type=StringField(Text_Line,6,Chr(59))
  ...
  ForEach Comp_Database()
     Compare_Map(Comp_Database()\C_Slave)=Comp_Database()\C_Type
  Next
  
  ForEach UM_Database()
    If FindMapElement(Compare_Map(),LCase(UM_Database()\UM_Slave))
      UM_Database()\UM_Type=Compare_Map()
    EndIf
  Next
...
Procedure Draw_List()
  ...
  Protected Text.s
  Protected Count
  Protected previous_entry.s, previous_path.s
  ...
  If ListSize(Filtered_List())>0
    ...
    ForEach Filtered_List()
      ...
      Select Short_Names
        Case #True
          Text=UM_Database()\UM_Short+Chr(10)
        Case #False
          Text=UM_Database()\UM_Name+Chr(10)
      EndSelect
      If Output_Case=1
        Text=LCase(Text)
      EndIf
      If Output_Case=2
        Text=UCase(Text)
      EndIf
      Text+UM_Database()\UM_Slave+Chr(10)+UM_Database()\UM_Path
      ;+Chr(10)+UM_Database()\UM_Slave+Chr(10)+UM_Database()\UM_Path+Chr(10)+UM_Database()\UM_Year+Chr(10)+UM_Database()\UM_Players+Chr(10)+UM_Database()\UM_Chipset
      AddGadgetItem(#MAIN_LIST,-1,text)
      If ListIndex(UM_Database())>1
        If previous_entry=UM_Database()\UM_Name
          If previous_path=UM_Database()\UM_Path
      ...
      previous_path=UM_Database()\UM_Path
    Next
    ...
    If GetWindowLongPtr_(GadgetID(#MAIN_LIST), #GWL_STYLE) & #WS_VSCROLL
      SetGadgetItemAttribute(#MAIN_LIST,3,#PB_ListIcon_ColumnWidth,385)
    Else
      SetGadgetItemAttribute(#MAIN_LIST,3,#PB_ListIcon_ColumnWidth,400)
    EndIf
...
Procedure Main_Window()
  ...
  If OpenWindow(#MAIN_WINDOW,0,0,900,620,"IGame Tool "+Version,#PB_Window_SystemMenu|#PB_Window_ScreenCentered)
    
    Pause_Window(#MAIN_WINDOW)
    
    ListIconGadget(#MAIN_LIST,0,0,900,550,"Name",300,#PB_ListIcon_GridLines|#PB_ListIcon_FullRowSelect|#PB_ListIcon_MultiSelect)
    SetGadgetColor(#MAIN_LIST,#PB_Gadget_BackColor,#White)
    AddGadgetColumn(#MAIN_LIST,1,"Slave",220)
    AddGadgetColumn(#MAIN_LIST,2,"Folder",200)
;     AddGadgetColumn(#MAIN_LIST,3,"Path",220)
;     AddGadgetColumn(#MAIN_LIST,4,"Year",120)
;     AddGadgetColumn(#MAIN_LIST,5,"Players",120)
;     AddGadgetColumn(#MAIN_LIST,6,"Chipset",120)
    ...
    If GetWindowLongPtr_(GadgetID(#MAIN_LIST), #GWL_STYLE) & #WS_VSCROLL
      SetGadgetItemAttribute(#MAIN_LIST,3,#PB_ListIcon_ColumnWidth,650)
    Else
      SetGadgetItemAttribute(#MAIN_LIST,3,#PB_ListIcon_ColumnWidth,665)
    EndIf
  ...
  Select event
    ...
    Case #PB_Event_CloseWindow
      ...
      If EventWindow()=#EDIT_WINDOW
        ...
        Define Text.s
        If Short_Names
          Text=UM_Database()\UM_Short+Chr(10)+UM_Database()\UM_Genre+Chr(10)+UM_Database()\UM_Slave+Chr(10)+UM_Database()\UM_Path+Chr(10)+UM_Database()\UM_Year+Chr(10)+UM_Database()\UM_Players+Chr(10)+UM_Database()\UM_Chipset
        Else
          Text=UM_Database()\UM_Name+Chr(10)+UM_Database()\UM_Genre+Chr(10)+UM_Database()\UM_Slave+Chr(10)+UM_Database()\UM_Path+Chr(10)+UM_Database()\UM_Year+Chr(10)+UM_Database()\UM_Players+Chr(10)+UM_Database()\UM_Chipset
        EndIf  
...
End
; IDE Options = PureBasic 6.21 Beta 6 (Windows - x64)
; CursorPosition = 372
; FirstLine = 75
; Folding = AAA+
; Optimizer
